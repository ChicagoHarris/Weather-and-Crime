...
testingData = read.csv(file = testingFile,sep=",",row.names=NULL)
...
#forecastData = read.csv(paste(testingDataDir, "WeatherandCrime_Data.robbery.",indexOfBaggedSamples,".binned.\
#csv",sep=""))
...
for (i in c(1:numOfBaggedSamples)){
  ...
  baggedData = read.csv(paste(binned_csvDir, "WeatherandCrime_Data.",crimeType,".",i,".binned.csv",sep=""), nrows=1)
  ...
  trainedNN = try(readRDS(file = paste(modelDir, "._NNmodel_NoWeather_", crimeType, "_", i, "_Update_", modelDate, ".rds", sep = "")))
...
write.csv(forecastData, file = paste(predictionDir,"prediction_noweather ",crimeType,".csv",sep = ""),row.names = FALSE)
print("Done.") 
