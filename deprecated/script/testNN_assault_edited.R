...
#forecastData = read.csv(paste(testingDataDir, "WeatherandCrime_Data.assault.",indexOfBaggedSamples,".binned.\
#csv",sep=""))
...
for (i in c(1:numOfBaggedSamples)){
  ...
  baggedData = read.csv(paste(testingDataDir, "WeatherandCrime_Data.assault.",i,".binned.csv",sep=""), nrows=1)
...
write.csv(forecastData, file = paste(predictionDir,"prediction_assault.csv",sep = ""),row.names = FALSE)
print("Done.") 
